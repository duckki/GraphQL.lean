...
@[simp] theorem resolvedValueOrNull_none :
    resolvedValueOrNull (ObjectRef := ObjectRef) none = .null := by
  rfl

@[simp] theorem resolvedValueOrNull_some (value : ResolverValue ObjectRef) :
    resolvedValueOrNull (some value) = value := by
  rfl

@[simp] theorem resolvedValueOrNull_option_null :
    resolvedValueOrNull (ObjectRef := ObjectRef) Option.null = .null := by
  rfl

@[simp] theorem resolvedValueOrNull_option_scalar (value : String) :
...
@[simp] theorem resolvedValueOrNull_option_list
