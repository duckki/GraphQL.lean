...
- Fidelity note: validation-time collection ignores executable directives and follows
  inline fragments by declared type condition without execution-time runtime filtering.
-/
namespace GraphQL
...
where
  ...
  fieldsForNameCanMerge (fuel : Nat) (left right : ScopedField) : Prop :=
    sameResponseShape schema left.outputType right.outputType
      ...
      ∧ fieldsInSetCanMerge schema fuel left.outputType.namedType
