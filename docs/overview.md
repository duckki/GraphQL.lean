...
- `GraphQL.Schema`: shared names, type references, input values, constant input values, built-in scalars, custom scalars, enums, objects, interfaces, unions, input objects, field definitions with output types, argument definitions with input types/defaults, field lookup, possible-object inclusion for abstract types, constant default-value validation, and output subtype checks.
- `GraphQL.SchemaWellFormedness`: schema-level invariants separated from raw schema syntax, including unique type/field/argument names, non-empty object/interface/union/enum/input-object definition lists, root query object type, valid type references and defaults, and object/interface implementation compatibility.
...
- `GraphQL.Validation`: validation as a proposition over a schema and operation, including variable definitions and defaults, variable-use compatibility at input locations, duplicate argument checks, required argument checks, recursive input/output type checks, non-empty required selection sets, modeled directive validation for `@skip`/`@include`, field merge checks, unique fragment names, spread resolution, acyclic fragment dependencies, and fragment applicability by possible-object overlap.
